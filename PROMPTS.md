...
### 사용자의 직접 기여가 구현을 바꾼 방식

- **직접 지시:** 사용자는 기술 스택, TypeScript strict, 상태 소유권, 기능별 독립 작업, 중간 문서 갱신과 검증 기준을 처음부터 지정했습니다. 가상화·Undo·DnD 같은 후속 범위도 사용자의 명시적 선택 뒤에만 추가했습니다.
- **직접 리뷰:** 완성된 결과를 추상적으로 평가하는 데 그치지 않고 `useTransition`, `memo`, `useLayoutEffect`, capture handler, Input 분기와 SearchBar JSX 등 실제 코드를 인용해 필요성·가독성·책임 위치를 질문했습니다.
- **직접 피드백:** 실패 UI와 성공 알림, 빈 검색 결과의 높이, 마지막 카드 잘림, Provider 중복, CloseButton 중복, DnD lifecycle 경고처럼 사용 중 또는 코드 검토 중 발견한 구체적인 문제를 전달했습니다.
- **수정 방향 제시:** 단순히 “고쳐 달라”고 하지 않고 동시성 범위 축소, Guard/Wrapper 사용, `react-error-boundary`와 `@dnd-kit` 검토, 도메인별 constants, 역할별 폴더, 구체적인 hook 이름 등 대안을 제시했습니다.
- **재검토와 정정:** 일반 element 이름과 맞추자는 원칙을 적용한 뒤에도 Radix Select의 원래 계약이 `onValueChange`임을 확인하고 유지하도록 정정했습니다. SearchBar 역시 `onClear` 존재 여부와 값 길이 조건까지 후속 피드백으로 좁혔습니다.
- **직접 기능 검증:** 구현 완료 뒤 모든 기능을 직접 테스트했다고 밝혔습니다. 에이전트의 자동 테스트·build·브라우저 검증과 별개의 사용자 검증으로 기록합니다.

아래 변경들은 이러한 사용자 기여를 받은 뒤 에이전트가 코드·테스트·문서를 확인해 그대로 채택하거나, 계약과 충돌하는 부분을 수정·기각한 결과입니다.
...
- 코드에서 지원자 조회 캐시, AbortSignal 취소, 재시도, 배경 갱신 상태, mutation lifecycle, 카드별 rollback이 TanStack Query 경계에 함께 있고 보드·요약·상세가 같은 Query 캐시를 읽는 것을 확인했습니다.
- React 공식 문서의 Action 진행 중 임시 상태 모델과 TanStack Query 공식 optimistic update 가이드를 대조했습니다. `useOptimistic`을 추가하면 동일한 카드 상태를 위한 별도 임시 계층이 생기므로 현재 구조에서는 중복이라고 판단했습니다.
- 구현 코드는 변경하지 않았습니다. 판단 근거는 README와 DECISIONS에 보강했고, 최신 코드에 `useTransition`이 없으며 검색 문자열의 `useDeferredValue`만 남아 있는지도 재확인해 README의 이전 설명을 정정했습니다.
