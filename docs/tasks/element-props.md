...
## 최종 계약
[기능 브랜치] / [기능 작업 공간]. Radix Select 원래 onValueChange 유지. 추가했던 onChange 변환과 Omit 제거, 실제 Toolbar도 원상 복원. Input/SearchBar native onChange(event), Button onClick/loading은 정상 확인. 의미별 도메인 액션과 라이브러리 원본 콜백 유지.
