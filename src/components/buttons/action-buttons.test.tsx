...
describe("semantic buttons", () => {
  ...
  it("keeps the retry label and focus while pending and excludes repeated clicks", async () => {
    const click = vi.fn();
    const { rerender } = render(
      <RetryButton onClick={click}>새로고침</RetryButton>,
    );
    const button = screen.getByRole("button", { name: "새로고침" });
    await userEvent.click(button);
    rerender(
      <RetryButton onClick={click} pending>
        새로고침
      </RetryButton>,
    );
    ...
    expect(click).toHaveBeenCalledTimes(1);
    rerender(
      <RetryButton onClick={click} disabled>
        새로고침
      </RetryButton>,
    );
    expect(button).toBeDisabled();
