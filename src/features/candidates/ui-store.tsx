...
const safeStorage: StateStorage = {
  ...
  setItem: (key, value) => { try { localStorage.setItem(key, value); } catch { /* Keep UI usable. */ } },
  removeItem: (key) => { try { localStorage.removeItem(key); } catch { /* Keep UI usable. */ } },
};
...
function validatedSettings(value: unknown): { search: string; job: string } {
  ...
  return {
    search: typeof settings.search === "string" ? settings.search.slice(0, 200) : "",
    job: settings.job === "all" || JOBS.some((job) => job === settings.job) ? settings.job as string : "all",
  };
}

function createUIStore() {
  return createStore<CandidateUIState>()(persist((set) => ({
    search: "", job: "all", selectedId: null, hydrated: false,
    ...
    merge: (saved, current) => ({ ...current, ...validatedSettings(saved) }),
  }));
}
