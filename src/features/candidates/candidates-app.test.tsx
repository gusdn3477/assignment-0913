...
const candidates: Candidate[] = [
  { id: "a", name: "김하늘", job: JOBS[0], stage: "review", appliedAt: "2026-09-12", email: "a@example.com", summary: "첫 번째 지원자" },
  { id: "b", name: "김여름", job: JOBS[1], stage: "interview", appliedAt: "2026-09-11", email: "b@example.com", summary: "두 번째 지원자" },
  { id: "c", name: "이봄", job: JOBS[0], stage: "offer", appliedAt: "2026-09-10", email: "c@example.com", summary: "세 번째 지원자" },
];
...
const mount = () => render(<Providers><CandidatesApp /></Providers>);
const detail = (name: string) => screen.queryByRole("button", { name: `${name} 지원자 상세 보기` });
...
describe("CandidatesApp acceptance", () => {
  it("shows initial loading until the API resolves, then renders all five columns", async () => {
    ...
    mount();
    expect(screen.getByRole("status", { name: "지원자를 불러오는 중" })).toBeInTheDocument();
    expect(screen.queryByRole("searchbox")).not.toBeInTheDocument();
    await act(async () => request.resolve(candidates));
    await screen.findByRole("searchbox");
    expect(screen.queryByRole("status", { name: "지원자를 불러오는 중" })).not.toBeInTheDocument();
    for (const stage of STAGES) expect(screen.getByRole("heading", { name: STAGE_LABELS[stage] })).toBeInTheDocument();
    expect(screen.getAllByRole("button", { name: /지원자 상세 보기/ })).toHaveLength(3);
  });

  it("displays a query failure and allows an explicit retry to recover", async () => {
    const retry = deferred<Candidate[]>();
    vi.mocked(candidateApi.listCandidates).mockRejectedValueOnce(new Error("목록 요청 실패")).mockReturnValueOnce(retry.promise);
    mount();
    expect(await screen.findByRole("alert")).toHaveTextContent("목록 요청 실패");
    expect(candidateApi.listCandidates).toHaveBeenCalledTimes(1);
    await userEvent.click(screen.getByRole("button", { name: "다시 불러오기" }));
    expect(await screen.findByRole("status", { name: "지원자를 불러오는 중" })).toBeInTheDocument();
  ...
  it("distinguishes an empty dataset from an empty filter result", async () => {
    ...
    mount();
    expect(await screen.findByText("아직 등록된 지원자가 없어요")).toBeInTheDocument();
    expect(screen.queryByText("검색 조건에 맞는 지원자가 없어요")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "검색 조건 초기화" })).not.toBeInTheDocument();
  ...
  it("composes name and job filters and resets a filtered empty result", async () => {
    ...
    expect(screen.getByText("검색 조건에 맞는 지원자가 없어요")).toBeInTheDocument();
    expect(screen.queryByText("아직 등록된 지원자가 없어요")).not.toBeInTheDocument();
  ...
  it("shows an optimistic move, locks its save control, then rolls back and shows the failure toast", async () => {
    ...
    mount();
    const move = await screen.findByRole("button", { name: "김하늘 단계 변경" });
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout", "setInterval", "clearInterval"] });
    ...
    expect(within(interview).getByRole("button", { name: "김하늘 지원자 상세 보기" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "김하늘 단계 변경 (저장 중)" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "김여름 단계 변경" })).toBeEnabled();
    expect(candidateApi.updateCandidateStage).toHaveBeenCalledExactlyOnceWith({ id: "a", stage: "interview" });
    await act(async () => {
      save.reject(new Error("저장 실패"));
      await vi.runOnlyPendingTimersAsync();
    });
    const review = screen.getByRole("region", { name: "서류검토 1명" });
    expect(within(review).getByRole("button", { name: "김하늘 지원자 상세 보기" })).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "면접 1명" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "김하늘 단계 변경" })).toBeEnabled();
    expect(screen.getByText("단계 이동을 저장하지 못했습니다. 다시 시도해 주세요.")).toBeInTheDocument();
  });
});
