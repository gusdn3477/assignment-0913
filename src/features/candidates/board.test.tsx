...
const candidate: Candidate = { id: "a", name: "김하늘", job: "프론트엔드 개발자", stage: "review", appliedAt: "2026-09-12T10:00:00Z", email: "test@example.com", summary: "지원자 소개" };
...
describe("CandidateBoard", () => {
  ...
  it("sorts by application date descending and id ascending without changing the input", () => {
    const candidates = [candidate, { ...candidate, id: "c", name: "이봄", appliedAt: "2026-09-13" }, { ...candidate, id: "b", name: "박여름", appliedAt: "2026-09-13" }];
    render(<CandidateBoard {...props} candidates={candidates} />);
    const buttons = screen.getAllByRole("button", { name: /상세 보기/ });
    expect(buttons.map((button) => button.dataset.candidateDetail)).toEqual(["b", "c", "a"]);
  ...
  it("opens details with keyboard and exposes separate move buttons", async () => {
    ...
    await user.tab(); 
    expect(screen.getByRole("button", { name: "김하늘 단계 변경" })).toHaveFocus();
  });
  ...
  it("disables the current stage and moves via keyboard", async () => {
    ...
    expect(screen.getByRole("menuitem", { name: /서류검토/ })).toHaveAttribute("data-disabled");
  ...
  it("blocks only the pending card's move control while keeping details available", () => {
    render(<CandidateBoard {...props} candidates={[candidate, { ...candidate, id: "b", name: "이봄" }]} pendingIds={new Set(["a"])} />);
    expect(screen.getByRole("button", { name: "김하늘 단계 변경 (저장 중)" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "이봄 단계 변경" })).toBeEnabled();
  ...
  it("does not steal external focus on unrelated data changes", () => {
    const { rerender } = render(<><input aria-label="검색" /><CandidateBoard {...props} /></>);
    screen.getByRole("button", { name: /상세 보기/ }).focus(); 
    screen.getByRole("textbox").focus();
    rerender(<><input aria-label="검색" /><CandidateBoard {...props} candidates={[{ ...candidate, stage: "interview" }]} /></>);
    expect(screen.getByRole("textbox")).toHaveFocus();
  });

  it("renders 250 candidates with distinct controls", () => {
    const candidates = Array.from({ length: 250 }, (_, index) => ({ ...candidate, id: String(index), stage: STAGES[index % STAGES.length] }));
    render(<CandidateBoard {...props} candidates={candidates} />);
    expect(screen.getAllByRole("button", { name: /상세 보기/ })).toHaveLength(250);
    for (const stage of STAGES) expect(screen.getByRole("region", { name: `${STAGE_LABELS[stage]} 50명` })).toBeInTheDocument();
  });
});
