...
## SearchBar 기본 조합 정정
- 사용자 정정에 따라 SearchBar가 Input left Search/right CloseButton을 직접 조합합니다. 사용처의 clearButton 설정 객체와 반복 스타일을 제거하고 native onChange와 선택적 onClear를 최종 계약으로 사용합니다.
- 공통 native Input의 슬롯/선택적 clearButton 계약은 유지합니다. SearchBar 지우기는 onClear 콜백을 호출하며 native ChangeEvent를 만들어 전달하지 않습니다. onClear가 있고 value가 비어 있지 않을 때 X를 표시하고 disabled/readOnly에서는 비활성화합니다. native value/defaultValue와 버튼 form semantics 및 ref/focus 동작을 유지합니다.
...
- react-error-boundary6.1.5를 직접 의존성으로 설치합니다. React18/19 peer 범위와 현재 React19/Next client 모듈 호환 확인.
- 공식 fallbackRender/resetErrorBoundary/onReset 패턴을 사용합니다. 앱 소유 class/failed state/getDerivedStateFromError/setState를 제거하고 기존 한글 안내/RetryButton/label만 조합합니다.
- onRecover를 onReset에 연결해 재렌더 전에 원인을 정리합니다. 상세 key 기반 재마운트는 유지하고 API 오류에 새 boundary를 강제하지 않습니다. 일반 재시도용 resetKeys나 미사용 추상화를 추가하지 않습니다.
